...
def render(ctx):
    ...
    if trades_list:
        ...
        s.append("</table><div class='note'>Δ=<b>FAで補強し尽くした場合と比べた</b>スタメン合計pt/週の改善幅(ブレンド値)。"
                 ...
                 "◎は双方プラスかつΔ優勝%も正(算出できた場合)で即打診可、○は相手が微マイナス(週2pt以内)なので売り文句付きで交渉。"
                 "Δ優勝%は上位案のみ、Δptと同じ「双方FA補強後」の前提でロスター入替後に再シミュレーションして算出。"
                 "|Δ優勝%|が2pt未満は再シミュレーションのノイズと区別できないため参考値、それを明確に下回る(大きくマイナスの)案は候補から除外済み。</div></div>")
    else:
